...
/datum/controller/subsystem/mapping/proc/load_station_room_templates()
	for(var/item in subtypesof(/datum/map_template/modular_room))
		...
		var/datum/map_template/modular_room/E = new room_type()
		map_templates[E.room_id] = E
		if(current_map.map_name == E.station_name)
			modular_room_templates[E.room_type] += list("[E.room_id]" = E)
			if(E.coordinates.len && E.template_width && E.template_height && !modular_room_spawners[E.room_type])
				create_room_spawner(E, E.room_type)
