...
/datum/pain/process(seconds_per_tick)
	if(!HAS_TRAIT(parent, TRAIT_ANALGESIA))
		...
		switch(shock_buildup * 0.25)
			if(10 to 60)
				parent.adjust_bodytemperature(-5 * seconds_per_tick, min_temp = BODYTEMP_COLD_DAMAGE_LIMIT + 5)
			if(60 to 120)
				if(SPT_PROB(2, seconds_per_tick))
				...
				parent.adjust_bodytemperature(-10 * seconds_per_tick, min_temp = BODYTEMP_COLD_DAMAGE_LIMIT - 5)
			if(120 to 180)
				if(SPT_PROB(2, seconds_per_tick))
				...
				if(SPT_PROB(4, seconds_per_tick))
					...
					parent.pain_emote("shiver", 3 SECONDS)
				parent.adjust_bodytemperature(-20 * seconds_per_tick, min_temp = BODYTEMP_COLD_DAMAGE_LIMIT - 20)
...
/datum/pain/proc/do_pain_emote(emote = pick(PAIN_EMOTES), cooldown = 3 SECONDS)
	...
	if(cooldown && !COOLDOWN_FINISHED(src, time_since_last_pain_message))
		return FALSE
	if(parent.stat >= UNCONSCIOUS || parent.incapacitated)
		return FALSE
