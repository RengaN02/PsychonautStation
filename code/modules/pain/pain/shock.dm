...
/datum/disease/acute/premade/shock/stage_act(seconds_per_tick, times_fired)
	. = ..()
	if(!ishuman(affected_mob))
		return
	...
	switch(stage)
		...
		if(1)
			...
			affected_mob.adjust_bodytemperature(-5 * seconds_per_tick, min_temp = BODYTEMP_COLD_DAMAGE_LIMIT + 5) // Not lethal
		...
		if(2)
			...
			if(SPT_PROB(8, seconds_per_tick))
				...
				affected_mob.pain_emote("shiver", 3 SECONDS)
			affected_mob.adjust_bodytemperature(-10 * seconds_per_tick, min_temp = BODYTEMP_COLD_DAMAGE_LIMIT - 5) // uh oh
		...
		if(3)
			...
			affected_mob.adjust_bodytemperature(-10 * seconds_per_tick, min_temp = BODYTEMP_COLD_DAMAGE_LIMIT - 20) // welp
